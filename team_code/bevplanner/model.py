...
class LidarCenterNet(nn.Module):
  ...
  def __init__(self, config):
    super().__init__()
    self.config = config
    self.bev_config = Config.fromfile("./config.py")  
    self.bev_planner = bevp.BevPlanner(self.bev_config.model)
  ...
  def forward(self, rgb, lidar_bev, target_point, ego_vel, command, img_meta=None):
    bs = rgb.shape[0]

    if self.config.backbone == 'transFuser':
      bev_feature_grid, fused_features, image_feature_grid = self.backbone(rgb, lidar_bev, img_meta)
    else:
      raise ValueError('The chosen vision backbone does not exist. '
                       'The options are: transFuser, aim, bev_encoder')
  ...
  @torch.no_grad()
  def cat_sixv(self, six_view_combined, white_background, ori_w=200, ori_h=112):
    # 分解 six_view_combined 得到六个视角
    front_row = six_view_combined[:, :, :ori_h, :]
    back_row = six_view_combined[:, :, ori_h:, :]

    # 分解各视角（根据拼接顺序）
    sixv_1 = front_row[:, :, :, :ori_w]
    sixv_0 = front_row[:, :, :, ori_w:ori_w*2]
    sixv_2 = front_row[:, :, :, ori_w*2:ori_w*3]
    sixv_4 = back_row[:, :, :, :ori_w]
    sixv_3 = back_row[:, :, :, ori_w:ori_w*2]
    sixv_5 = back_row[:, :, :, ori_w*2:ori_w*3]

    # 对指定视角旋转
    sixv_1_rot = torch.rot90(sixv_1, k=1, dims=(2, 3))  # 逆时针90°
    sixv_4_rot = torch.rot90(sixv_4, k=1, dims=(2, 3))
    sixv_2_rot = torch.rot90(sixv_2, k=3, dims=(2, 3))  # 顺时针90°
    sixv_5_rot = torch.rot90(sixv_5, k=3, dims=(2, 3))
    ...
    def pad_to_width(tensor, target_width):
        ...
        return torch.nn.functional.pad(tensor, (pad_left, pad_right, 0, 0))

    # 填充中间行（sixv_0和sixv_3）
    sixv_0_padded = pad_to_width(sixv_0, ori_h*2)
    sixv_3_padded = pad_to_width(sixv_3, ori_h*2)

    # 按行拼接
    row1 = sixv_0_padded
    row2 = torch.cat([sixv_1_rot, sixv_2_rot], dim=3)
    row3 = torch.cat([sixv_4_rot, sixv_5_rot], dim=3)
    row4 = sixv_3_padded

    # 垂直拼接所有行
    ring_view = torch.cat([row1, row2, row3, row4], dim=2)

    # 调整尺寸为6视角的尺寸（200x112）
    ring_view_resized = torch.nn.functional.interpolate(
        ring_view, size=(1280, 676), mode='bilinear', align_corners=False
    )

    # 转换到CPU并转为numpy数组
    ring_view_np = ring_view_resized.squeeze(0).permute(1, 2, 0).cpu().numpy().astype(np.uint8)

    # 拼接白色背景和环视图
    final_image = np.concatenate([white_background, ring_view_np], axis=1)
    return final_image

  @torch.no_grad()
  def cat_sixv2(self, six_view_combined, white_background):
    # 调整尺寸为6视角的尺寸（200x112）
    ring_view_resized = torch.nn.functional.interpolate(
        six_view_combined, size=(382, 1024), mode='bilinear', align_corners=False
    )
    # 转换到CPU并转为numpy数组
    ring_view_np = ring_view_resized.squeeze(0).permute(1, 2, 0).cpu().numpy().astype(np.uint8)

    # 拼接白色背景和环视图
    # print(ring_view_np.shape, white_background.shape)
    final_image = np.concatenate([white_background, ring_view_np], axis=0)
    return final_image
  ...
  def visualize_model(  # pylint: disable=locally-disabled, unused-argument
    ...
    white_background = np.full_like(road[:, :, :3], 255, dtype=np.uint8)
    # Alpha blending the road over the LiDAR
    images_lidar = road[:, :, 3:4] * road[:, :, :3] + (1-road[:, :, 3:4]) * white_background
    ...
    if pred_checkpoint is not None and False:
    ...
    rgb_image = cv2.resize(rgb[0].permute(1, 2, 0).detach().cpu().numpy(), (1024, 512))
    rgb_image = rgb_image

    if wp_selected is not None and False:
    ...
    if pred_speed is not None and False:
      pred_speed = pred_speed.detach().cpu().numpy()[0]
      images_lidar = np.ascontiguousarray(images_lidar, dtype=np.uint8)
      t_u.draw_probability_boxes(images_lidar, pred_speed, self.config.target_speeds)
    front_row = torch.cat([lidar_bev[0], lidar_bev[1], lidar_bev[2]], dim=3)
    back_row = torch.cat([lidar_bev[3], lidar_bev[4], lidar_bev[5]], dim=3)
    lidar_bev = torch.cat([front_row, back_row], dim=2)
    all_images = np.concatenate((rgb_image, images_lidar), axis=0)
    all_images = self.cat_sixv2(lidar_bev, all_images)
