...
import copy
import bevplanner.bev_planner as bevp
from mmcv import Config
from bevplanner import get_camparams
...
class TransfuserBackbone(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.config = config
    self.config = config
    self.bev_config = Config.fromfile("/media/czg/data_store/TFbevplanner/team_code/bevplanner/config.py")
    self.bev_planner = bevp.BEVPlanner(forward_projection=self.bev_config.model.forward_projection, \
                                       frpn=self.bev_config.model.frpn, \
                                       depth_net=self.bev_config.model.depth_net, \
                                       single_bev_num_channels=self.bev_config.model.single_bev_num_channels, \
                                       use_grid_mask=self.bev_config.model.use_grid_mask, \
                                       with_ego_status=self.bev_config.model.with_ego_status, \
                                       align_prev_bev=self.bev_config.model.align_prev_bev, \
                                       img_backbone=self.bev_config.model.img_backbone, \
                                       img_neck=self.bev_config.model.img_neck, \
                                       add_forward_backbward_feats=True, \
                                       fix_void=self.bev_config.fix_void, \
                                       do_history=self.bev_config.do_history, \
                                       fuse_history_bev=self.bev_config.model.fuse_history_bev, \
                                       history_cat_num=self.bev_config.history_cat_num, \
                                       history_cat_conv_out_channels=self.bev_config.history_cat_conv_out_channels, \
                                       embed_dims=self.bev_config.embed_dims)
    ...
    if self.config.detect_boxes or self.config.use_bev_semantic:
      ...
      self.cam_params = get_camparams.get_sensor_params()
      self.cam_params_init = True
  ...
  def forward(self, image, lidar, img_metas):
    ...
    if self.cam_params_init:
      self.cam_params_init = False
      for i in range(len(self.cam_params)):
          self.cam_params[i] = torch.tensor([self.cam_params[i] for _ in range(bs)], dtype=torch.float32)
    cam_params = self.cam_params
    for i in range(len(cam_params)):
      cam_params[i] = cam_params[i].to(image.device)
    lidar = torch.stack(lidar, dim=1)
    lidar_input = [lidar]+cam_params
    bev_feat = self.bev_planner.extract_img_bev_feat(lidar_input, img_metas)
    ...
    if self.lidar_video:
      ...
      lidar_features = bev_feat['img_bev_feat'][0]
