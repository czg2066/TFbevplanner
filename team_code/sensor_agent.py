...
class SensorAgent(autonomous_agent.AutonomousAgent):
  ...
  def setup(self, path_to_conf_file, route_index=None):
    ...
    if self.save_path is not None and route_index is not None:
      ...
      self.save_path = None
    # self.nuSoc = oc.nuScenes_set(1,0,0,0,0)
    self.nuSoc = oc.Geely_sensors(1, 0, 0, 0)
  ...
  def sensors(self):
    ...
    sensors += self.nuSoc.sensors_set
  ...
  def tick(self, input_data):
    ...
    rgb = []
    # for camera_pos in ['front']+self.nuSoc.rgb:
    #   if camera_pos == 'front':
    #     rgb_cam = 'rgb_' + camera_pos
      # else:
    for camera_pos in ["rgb_front_tele", "rgb_front_left", "rgb_front_wide", "rgb_front_right", "rgb_rear_left", "rgb_rear", "rgb_rear_right"] :
      rgb_cam = camera_pos
      ...
      camera = cv2.resize(camera, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
      ...
      rgb.append(rgb_pos)
    sixv_rgb = rgb[1:]
    rgb = np.concatenate([rgb[0]], axis=1)
    rgb = torch.from_numpy(rgb).to(self.device, dtype=torch.float32).unsqueeze(0)
    for i in range(len(sixv_rgb)):
      # sixv_rgb[i] = np.concatenate(sixv_rgb[i], axis=1)
      sixv_rgb[i] = torch.from_numpy(sixv_rgb[i]).to(self.device, dtype=torch.float32).unsqueeze(0)
    # front_row = torch.cat([sixv_rgb[1], sixv_rgb[0], sixv_rgb[2]], dim=3)
    # back_row = torch.cat([sixv_rgb[4], sixv_rgb[3], sixv_rgb[5]], dim=3)
    # six_view_combined = torch.cat([front_row, back_row], dim=2)accelerometer
    accelerometer = input_data['imu'][1][:3]
    gyroscope = input_data['imu'][1][3:6]
    # position = input_data['gps'][1][:]
    # yaw, pitch, roll = gyroscope
    ego_transform = self.lon_logger.ego_vehicle.get_transform()
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    bounding_boxes = []
    wp_selected = None
    img_meta =  self.data_converter.process_frames_with_can_enhancement(tick_data['img_meta'], self.step)
    img_meta = {k: v.unsqueeze(0) if isinstance(v, torch.Tensor) else torch.tensor(v).unsqueeze(0)
    ...
    for i in range(self.model_count):
      if self.config.backbone in ('transFuser', 'aim', 'bev_encoder'):
        ...
        selected_path = self.nets[i].forward(
          rgb=tick_data['rgb'],
          lidar_bev=tick_data['six_view_combined'],
    ...
    elif 1:#self.config.use_wp_gru and not self.config.inference_direct_controller:
