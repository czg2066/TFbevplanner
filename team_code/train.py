...
class Engine(object):
  ...
  def load_data_compute_loss(self, data, validation=False):
    ...
    if self.config.use_plant:
      ...
      rgb_back = data['rgb_back'].to(self.device, dtype=torch.float32)
      rgb_back_left = data['rgb_back_left'].to(self.device, dtype=torch.float32)
      rgb_back_right = data['rgb_back_right'].to(self.device, dtype=torch.float32)
      ...
      lidar_imgs = [rgb_front_left, rgb_front, rgb_front_right, rgb_back_left, rgb_back, rgb_back_right]
      for i in range(len(lidar_imgs)):
        lidar_imgs[i] = lidar_imgs[i].to(self.device, torch.float32) 
  ...
  def train(self):
    ...
    # Train loop
    print(f"DataLoader length: {len(self.dataloader_train)}")
    self.step = 0
    for i, data in enumerate(tqdm(self.dataloader_train, disable=self.rank != 0)):
      ...
      step_log = True
      if step_log:
        self.log_losses(loss_epoch, detailed_losses_epoch, num_batches, 'step_', self.step)
      self.step += 1
  ...
  def log_losses(self, loss_epoch, detailed_losses_epoch, num_batches, prefix='', step=None):
    # Collecting the losses from all GPUs has led to issues.
    # I simply log the loss from GPU 0 for now they should be similar.
    if self.rank == 0:
      self.writer.add_scalar(prefix + 'loss_total', loss_epoch / num_batches, self.cur_epoch if step is None else step)

      for key, value in detailed_losses_epoch.items():
        self.writer.add_scalar(prefix + key, value / num_batches, self.cur_epoch if step is None else step)
