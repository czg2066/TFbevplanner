...
class CARLA_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
  ...
  def __init__(self,
               ...
               rank=0):
    self.config = config
    self.data_converter = cc.canbus_dataconverter()
    ...
    self.images = []
    self.imgs_front = []
    self.imgs_front_left = []
    self.imgs_front_right = []
    self.imgs_back = []
    self.imgs_back_left = []
    self.imgs_back_right = []
        ...
        for seq in range(config.skip_first, num_seq - self.config.pred_len - self.config.seq_len):
          ...
          for idx in range(self.config.seq_len):
            if not self.config.use_plant:
              image.append(route_dir + '/rgb_front_tele' + (f'/{(seq + idx):04}.jpg'))
              img_front.append(route_dir + '/rgb_front_wide' + (f'/{(seq + idx):04}.jpg'))
              img_front_left.append(route_dir + '/rgb_front_left' + (f'/{(seq + idx):04}.jpg'))
              img_front_right.append(route_dir + '/rgb_front_right' + (f'/{(seq + idx):04}.jpg'))
              img_back.append(route_dir + '/rgb_rear' + (f'/{(seq + idx):04}.jpg'))
              img_back_left.append(route_dir + '/rgb_rear_left' + (f'/{(seq + idx):04}.jpg'))
              img_back_right.append(route_dir + '/rgb_rear_right' + (f'/{(seq + idx):04}.jpg'))
              image_augmented.append(route_dir + '/rgb_augmented' + (f'/{(seq + idx):04}.jpg'))
              semantic.append(route_dir + '/semantic_front_tele' + (f'/{(seq + idx):04}.png'))
          ...
          self.imgs_front.append(img_front)
          self.imgs_front_left.append(img_front_left)
          self.imgs_front_right.append(img_front_right)
          self.imgs_back.append(img_back)
          self.imgs_back_left.append(img_back_left)
          self.imgs_back_right.append(img_back_right)
    ...
    self.imgs_front = np.array(self.imgs_front).astype(np.string_)
    self.imgs_front_left = np.array(self.imgs_front_left).astype(np.string_)
    self.imgs_front_right = np.array(self.imgs_front_right).astype(np.string_)
    self.imgs_back = np.array(self.imgs_back).astype(np.string_)
    self.imgs_back_left = np.array(self.imgs_back_left).astype(np.string_)
    self.imgs_back_right = np.array(self.imgs_back_right).astype(np.string_)
  ...
  def __getitem__(self, index):
    ...
    imgs_front = self.imgs_front[index]
    imgs_front_left = self.imgs_front_left[index]
    imgs_front_right = self.imgs_front_right[index]
    imgs_back = self.imgs_back[index]
    imgs_back_left = self.imgs_back_left[index]
    imgs_back_right = self.imgs_back_right[index]
    ...
    loaded_images = []
    loaded_imgs_front = []
    loaded_imgs_front_left = []
    loaded_imgs_front_right = []
    loaded_imgs_back = []
    loaded_imgs_back_left = []
    loaded_imgs_back_right = []
    ...
    for i in range(self.config.seq_len):
      ...
      if not self.data_cache is None and cache_key in self.data_cache:
        boxes_i, future_boxes_i, images_i, imgs_front_i, imgs_front_left_i, imgs_front_right_i, imgs_back_i, imgs_back_left_i, imgs_back_right_i,\
        images_augmented_i, semantics_i, semantics_augmented_i, bev_semantics_i,\
        bev_semantics_augmented_i, depth_i, depth_augmented_i = self.data_cache[cache_key]
        # , lidars_i
        if not self.config.use_plant:
          images_i = cv2.imdecode(images_i, cv2.IMREAD_UNCHANGED)
          imgs_front_i = cv2.imdecode(imgs_front_i, cv2.IMREAD_UNCHANGED)
          imgs_front_left_i = cv2.imdecode(imgs_front_left_i, cv2.IMREAD_UNCHANGED)
          imgs_front_right_i = cv2.imdecode(imgs_front_right_i, cv2.IMREAD_UNCHANGED)
          imgs_back_i = cv2.imdecode(imgs_back_i, cv2.IMREAD_UNCHANGED)
          imgs_back_left_i = cv2.imdecode(imgs_back_left_i, cv2.IMREAD_UNCHANGED)
          imgs_back_right_i = cv2.imdecode(imgs_back_right_i, cv2.IMREAD_UNCHANGED)
      ...
      # Load data from the disc
        ...
        images_i = None
        imgs_front_i = None
        imgs_front_left_i = None
        imgs_front_right_i = None
        imgs_back_i = None
        imgs_back_left_i = None
        imgs_back_right_i = None
        ...
        if not self.config.use_plant:
          ...
          images_i = cv2.resize(images_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
          images_i = cv2.cvtColor(images_i, cv2.COLOR_BGR2RGB)
          imgs_front_i = cv2.imread(str(imgs_front[i], encoding='utf-8'), cv2.IMREAD_COLOR)
          imgs_front_i = cv2.resize(imgs_front_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
          imgs_front_i = cv2.cvtColor(imgs_front_i, cv2.COLOR_BGR2RGB)
          imgs_front_left_i = cv2.imread(str(imgs_front_left[i], encoding='utf-8'), cv2.IMREAD_COLOR)
          imgs_front_left_i = cv2.resize(imgs_front_left_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
          imgs_front_left_i = cv2.cvtColor(imgs_front_left_i, cv2.COLOR_BGR2RGB)
          imgs_front_right_i = cv2.imread(str(imgs_front_right[i], encoding='utf-8'), cv2.IMREAD_COLOR)
          imgs_front_right_i = cv2.resize(imgs_front_right_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
          imgs_front_right_i = cv2.cvtColor(imgs_front_right_i, cv2.COLOR_BGR2RGB)
          imgs_back_i = cv2.imread(str(imgs_back[i], encoding='utf-8'), cv2.IMREAD_COLOR)
          imgs_back_i = cv2.resize(imgs_back_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
          imgs_back_i = cv2.cvtColor(imgs_back_i, cv2.COLOR_BGR2RGB)
          imgs_back_left_i = cv2.imread(str(imgs_back_left[i], encoding='utf-8'), cv2.IMREAD_COLOR)
          imgs_back_left_i = cv2.resize(imgs_back_left_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
          imgs_back_left_i = cv2.cvtColor(imgs_back_left_i, cv2.COLOR_BGR2RGB)
          imgs_back_right_i = cv2.imread(str(imgs_back_right[i], encoding='utf-8'), cv2.IMREAD_COLOR)
          imgs_back_right_i = cv2.resize(imgs_back_right_i, (self.img_h, self.img_w), interpolation=cv2.INTER_AREA)
        ...
        if not self.data_cache is None:
          ...
          compressed_img_front_i = None
          compressed_img_front_left_i = None
          compressed_img_front_right_i = None
          compressed_img_back_i = None
          compressed_img_back_left_i = None
          compressed_img_back_right_i = None
          ...
          if not self.config.use_plant:
            ...
            _, compressed_img_front_i = cv2.imencode('.jpg', imgs_front_i)
            _, compressed_img_front_left_i = cv2.imencode('.jpg', imgs_front_left_i)
            _, compressed_img_front_right_i = cv2.imencode('.jpg', imgs_front_right_i)
            _, compressed_img_back_i = cv2.imencode('.jpg', imgs_back_i)
            _, compressed_img_back_left_i = cv2.imencode('.jpg', imgs_back_left_i)
            _, compressed_img_back_right_i = cv2.imencode('.jpg', imgs_back_right_i)
            if self.config.use_semantic:
          ...
          self.data_cache[cache_key] = (boxes_i, future_boxes_i, compressed_image_i, compressed_img_front_i, compressed_img_front_left_i,\
                                       compressed_img_front_right_i, compressed_img_back_i, compressed_img_back_left_i, compressed_img_back_right_i,\
                                        ...
                                        compressed_depth_i, compressed_depth_augmented_i)
                                        # , compressed_lidar_i)

      loaded_images.append(images_i)
      loaded_imgs_front.append(imgs_front_i)
      loaded_imgs_front_left.append(imgs_front_left_i)
      loaded_imgs_front_right.append(imgs_front_right_i)
      loaded_imgs_back.append(imgs_back_i)
      loaded_imgs_back_left.append(imgs_back_left_i)
      loaded_imgs_back_right.append(imgs_back_right_i)
    ...
    if not self.config.use_plant:
      if self.config.augment and augment_sample:
        if self.config.use_color_aug:
          ...
          processed_img_front = self.image_augmenter_func(image=loaded_imgs_front[self.config.seq_len - 1])
          processed_img_front_left = self.image_augmenter_func(image=loaded_imgs_front_left[self.config.seq_len - 1])
          processed_img_front_right = self.image_augmenter_func(image=loaded_imgs_front_right[self.config.seq_len - 1])
          processed_img_back = self.image_augmenter_func(image=loaded_imgs_back[self.config.seq_len - 1])
          processed_img_back_left = self.image_augmenter_func(image=loaded_imgs_back_left[self.config.seq_len - 1])
          processed_img_back_right = self.image_augmenter_func(image=loaded_imgs_back_right[self.config.seq_len - 1])
        else:
          processed_image = loaded_images_augmented[self.config.seq_len - 1]
          processed_img_front = loaded_imgs_front[self.config.seq_len - 1]
          processed_img_front_left = loaded_imgs_front_left[self.config.seq_len - 1]
          processed_img_front_right = loaded_imgs_front_right[self.config.seq_len - 1]
          processed_img_back = loaded_imgs_back[self.config.seq_len - 1]
          processed_img_back_left = loaded_imgs_back_left[self.config.seq_len - 1]
          processed_img_back_right = loaded_imgs_back_right[self.config.seq_len - 1]
      ...
      data['rgb_front'] = np.transpose(processed_img_front, (2, 0, 1))
      data['rgb_front_left'] = np.transpose(processed_img_front_left, (2, 0, 1))
      data['rgb_front_right'] = np.transpose(processed_img_front_right, (2, 0, 1))
      data['rgb_back'] = np.transpose(processed_img_back, (2, 0, 1))
      data['rgb_back_left'] = np.transpose(processed_img_back_left, (2, 0, 1))
      data['rgb_back_right'] = np.transpose(processed_img_back_right, (2, 0, 1))
    ...
    data['measurements'] = self.data_converter.process_frames_with_can_enhancement(current_measurement, index)
