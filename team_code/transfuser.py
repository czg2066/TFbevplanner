...
import copy
import Backone
from bevformer.bevformer_head import BEVFormerHead
import imgmatrix
...
#         nn.Conv2d(in_channels, target_channels, 1),  # 升维到目标通道
#         nn.BatchNorm2d(target_channels),
#         nn.ReLU(inplace=True)
#     )
#     self.downnet = nn.Sequential(
#         nn.Conv2d(target_channels, in_channels, 1), # 恢复原始通道
...
class TransfuserBackbone(nn.Module):
  ...
  def __init__(self, config):
    super().__init__()
    self.config = config
    # self.bevviews_backone = Backone.RegNetBiFPN(
    ...
    self.bev_backbone = timm.create_model(
        'regnety_032',
        pretrained=True,
        features_only=True,
    )
    ...
    # self.backbone_output_channels = [32] + self.backbone_output_channels
    ...
    if self.lidar_video:
      self.lidar_channel_to_img = nn.ModuleList([
          nn.Conv3d(self.backbone_output_channels[start_index + i],
      ...
      self.img_channel_to_lidar = nn.ModuleList([
          nn.Conv3d(self.image_encoder.feature_info.info[start_index + i]['num_chs'],
                    self.backbone_output_channels[start_index + i],
                    kernel_size=1) for i in range(4)
      ])

    else:
      self.lidar_channel_to_img = nn.ModuleList([
          nn.Conv2d(self.backbone_output_channels[start_index + i],
                    self.image_encoder.feature_info.info[start_index + i]['num_chs'],
                    kernel_size=1) for i in range(4)
      ])
      self.img_channel_to_lidar = nn.ModuleList([
    ...
    if self.config.transformer_decoder_join:
      self.num_features = self.backbone_output_channels[start_index + 3]
    else:
      if self.config.add_features:
        self.lidar_to_img_features_end = nn.Linear(self.backbone_output_channels[start_index + 3],
        ...
        # Number of features the encoder produces.
        self.num_features = self.image_encoder.feature_info.info[start_index + 3]['num_chs'] + \
                            self.backbone_output_channels[start_index + 3]
    ...
    if self.config.detect_boxes or self.config.use_bev_semantic:
      ...
      # lateral
      self.c5_conv = nn.Conv2d(self.backbone_output_channels[start_index + 3], channel, (1, 1))

    transformer_dict = self.config.transformer_dict
    self.bev_embed_dims = self.image_encoder.feature_info.info[start_index-1]['num_chs']
    transformer_dict['embed_dims'] = self.bev_embed_dims
    transformer_dict['encoder']['transformerlayers']['attn_cfgs'][0]['embed_dims'] = self.bev_embed_dims
    transformer_dict['encoder']['transformerlayers']['attn_cfgs'][1]['embed_dims'] = self.bev_embed_dims
    transformer_dict['encoder']['transformerlayers']['attn_cfgs'][1]['deformable_attention']['embed_dims'] = self.bev_embed_dims
    transformer_dict['encoder']['transformerlayers']['ffn_cfgs']['embed_dims'] = self.bev_embed_dims
    transformer_dict['encoder']['transformerlayers']['ffn_cfgs']['feedforward_channels'] = self.bev_embed_dims*4
    transformer_dict['encoder']['transformerlayers']['feedforward_channels'] = self.bev_embed_dims*2
    self.bev_head =  BEVFormerHead(bev_h=self.config.bev_h_, bev_w=self.config.bev_w_, transformer_dict=transformer_dict, \
                          in_channels=self.bev_embed_dims, num_classes=self.config.num_bev_semantic_classes, \
                          embed_dims=self.bev_embed_dims)
    self.imgmatrixcals = imgmatrix.Lidar2ImgCalculator(config).get_lidar2img()
    # self.c0_prof  =  FeatureAdapter(self.backbone_output_channels[start_index + 0], self.num_image_features)
    # self.c1_prof  =  FeatureAdapter(self.backbone_output_channels[start_index + 1], self.num_image_features)
    # self.c2_prof  =  FeatureAdapter(self.backbone_output_channels[start_index + 2], self.num_image_features)
    # self.c3_prof  =  FeatureAdapter(self.backbone_output_channels[start_index + 3], self.num_image_features)
  ...
  def forward(self, image, lidar, img_metas):
    '''
        ...
        '''
    # bevviews_features = self.bevviews_backone(lidar)
    # print('lidar shape:', lidar[-1].shape)
    bs, _, _,  _ = lidar[-1].shape
    img_metas['lidar2img'] = [self.imgmatrixcals for _ in range(bs)]
    img_metas['img_shape'] = [[(self.config.img_w, self.config.img_h, 3) for __ in range(6)] for _ in range(bs)]
    ...
    lidar_layers = iter(self.backbone.items())
    lidar = torch.cat(lidar, dim=0)
    # lidar = self.bev_backbone(lidar)[-1]
    # Stem layer.
    ...
    if len(self.backbone.return_layers) > 4:
      lidar_features = self.forward_layer_block(lidar_layers, self.backbone.return_layers, lidar)
      lidar_features = self.bev_head(tuple([lidar_features.view(bs, 6, *lidar_features.shape[1:])]), img_metas)
      lidar_features = lidar_features.permute(0, 2, 1).view(bs, -1, self.config.bev_w_, self.config.bev_h_)
      lidar_features = F.interpolate(lidar_features, size=(lidar_features.shape[-2], lidar_features.shape[-1]), mode='bilinear', align_corners=False)
      # print('lidar_features shape:', lidar_features.shape, lidar_features_bev.shape)
      # lidar_features = lidar_features + lidar_features_bev
    # if len(self.lidar_encoder.return_layers) > 4:
    #   lidar_features = self.forward_layer_block(lidar_layers, self.lidar_encoder.return_layers, lidar_features) #(B, 32, 128, 128)

    # Loop through the 4 blocks of the network.
    for i in range(4):
      image_features = self.forward_layer_block(image_layers, self.image_encoder.return_layers, image_features)
      lidar_features = self.forward_layer_block(lidar_layers, self.backbone.return_layers, lidar_features) #(B, 72, 64, 64) (B, 216, 32, 32) (2, 576, 16, 16) (2, 1512, 8, 8)
      # lidar_features = lidar_features.permute(0, 2, 1).view(bs, 6, -1, -1) 
      # print('lidar_features shape:', lidar_features.shape)
      # lindar_c = lidar_features.shape[1]
      # if lindar_c == 72:  lidar_features = self.c0_prof.up(lidar_features)
      # elif lindar_c == 216: lidar_features = self.c1_prof.up(lidar_features)
      # elif lindar_c == 576: lidar_features = self.c2_prof.up(lidar_features)
      # elif lindar_c == 1512: lidar_features = self.c3_prof.up(lidar_features)
      # print('lidar_features shape:', lidar_features.shape)
      # lidar_features = self.bev_head(tuple([lidar_features.view(bs, 6, self.num_image_features, *lidar_features.shape[2:])]), img_metas)
      # print('lidar_features shape:', lidar_features.shape)
      # if lindar_c == 72:  lidar_features = self.c0_prof.down(lidar_features)
      # elif lindar_c == 216: lidar_features = self.c1_prof.down(lidar_features)
      # elif lindar_c == 576: lidar_features = self.c2_prof.down(lidar_features)
      # elif lindar_c == 1512: lidar_features = self.c3_prof.down(lidar_features)
      # lidar_features = lidar_features.permute(0, 2, 1).view(bs, -1, self.config.bev_w_, self.config.bev_h_)
