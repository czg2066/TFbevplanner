...
class GlobalConfig:
  ...
  def __init__(self):
        ...
        encoder=dict(
            ...
            transformerlayers=dict(
                ...
                operation_order=('self_attn', 'norm', 'cross_attn', 'norm',
                                    'ffn', 'norm'))))
    self.positional_encoding=dict(
        type='LearnedPositionalEncoding',
        num_feats=128,
        row_num_embed=self.bev_h_,
        col_num_embed=self.bev_w_,
        )
    ...
    self.epochs = 51  # Number of epochs to train
    ...
    self.root_dir = "/media/zr/Temp_data/geely_data"
    self.train_towns = []
    self.val_towns = []
    self.train_data = []
    self.val_data = []
  ...
  def initialize(self, root_dir='', setting='all', **kwargs):
    ...
    for town in self.train_towns:
      ...
      for file in root_files:
        if not os.path.isdir(os.path.join(self.root_dir, town, file)):
          continue
